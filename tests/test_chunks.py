...
import random
import string
from dewy_client.api.kb import add_collection, add_document, get_document_status, list_chunks, upload_document_content
...
from tests.test_documents import DocFixture
...
async def _add_nearly_empty_doc(client, collection_name) -> int:
    doc = await add_document.asyncio(client=client, body=AddDocumentRequest(collection=collection_name))
...
async def chunk_fixture(client) -> ChunkFixture:
    ...
    docs = await asyncio.gather(_add_nearly_empty_doc(client, collection_name),
...
async def test_list_chunks_in_collection(client, chunk_fixture):
    chunks = await list_chunks.asyncio(
        client=client, collection=chunk_fixture.collection_name,
    )
...
async def test_list_chunks_in_document(client, chunk_fixture):
    chunks = await list_chunks.asyncio(
        client=client, document_id=chunk_fixture.doc1,
    )
...
async def test_list_chunks_in_collection_case_insensitive(client, chunk_fixture):
    ...
    chunks = await list_chunks.asyncio(
        client=client, collection=upper,
    )
    assert {c.document_id for c in chunks} == {chunk_fixture.doc1, chunk_fixture.doc2}
...
async def test_list_chunks_invalid_collection(client):
    response = await list_chunks.asyncio(
        client=client, collection="invalid collection",
    )
    assert response == []

async def test_list_chunks_invalid_document(client, chunk_fixture):
    response = await list_chunks.asyncio(
        client=client, document_id=1_000_000,
    )
    assert response == []
